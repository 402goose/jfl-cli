...
async function executeServiceTool(serviceName: string, service: Service, toolName: string, args: any): Promise<string> {
  // Check for custom .jfl-mcp.js file first
  const servicePath = (service as any).path
  if (servicePath) {
...
async function executeCustomMCP(mcpPath: string, toolName: string, args: any): Promise<string> {
...
async function executeAutoGeneratedTool(serviceName: string, service: Service, toolName: string, args: any): Promise<string> {
  ...
  if (service.mcp?.tools) {
    const customTool = service.mcp.tools.find((t: any) => t.name === toolName)
    if (customTool) {
...
async function handleStatus(serviceName: string, service: Service): Promise<string> {
...
async function handleLogs(serviceName: string, service: Service, lines: number): Promise<string> {
  const vars: Record<string, string> = {
    PORT: service.port?.toString() || "",
    HOME: homedir(),
  }
...
async function handleHealth(serviceName: string, service: Service): Promise<string> {
...
async function executeCustomTool(serviceName: string, service: Service, tool: any, args: any): Promise<string> {
  const vars: Record<string, string> = {
    SERVICE_NAME: serviceName,
    PORT: service.port?.toString() || "",
    HOME: homedir(),
    SERVICE_PATH: (service as any).path || process.cwd(),
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (logsMatch && req.method === "GET") {
      ...
      try {
        ...
        const service: Service | undefined = globalServices.services[serviceName] || projectServices.services[serviceName]
